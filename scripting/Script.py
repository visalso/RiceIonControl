...
class Script(QtCore.QThread):
    ...
    saveCameraSignal = QtCore.pyqtSignal()
    startAcquireCameraSignal = QtCore.pyqtSignal()
    stopAcquireCameraSignal = QtCore.pyqtSignal()
    detectIonsCameraSignal = QtCore.pyqtSignal()
    ...
    setFileNameCameraSignal = QtCore.pyqtSignal(str) #args: name of the file set
    ...
    setAnalysisInitialSignal = QtCore.pyqtSignal(int, float) #args: analysis name, initial parameter
    ...
    checkTraceSignal = QtCore.pyqtSignal(str) #arg: name to check Trace Signal
    uncheckTraceSignal = QtCore.pyqtSignal(str) #arg: name to check Trace Signal
    ...
    checkPulseVariableSignal = QtCore.pyqtSignal(str, int) #args: name of variable, check state
    commentOutPulseLinesSignal = QtCore.pyqtSignal(int, int) #args: start line and end line numbers
    unCommentOutPulseLinesSignal = QtCore.pyqtSignal(int, int) #args: start line and end line numbers
    checkAcquireWithCameraSignal = QtCore.pyqtSignal(bool) #args: state of checkAcquireWithCamera
    checkAWGRowSignal = QtCore.pyqtSignal(str, str) #args: AWG name, name of the awg row
    uncheckAWGRowSignal = QtCore.pyqtSignal(str, str) #args: AWG name, name of the awg row
    initializeAWGSignal = QtCore.pyqtSignal(str) #args: AWG name
    setAWGVariableSignal = QtCore.pyqtSignal(str, str, str) #args: AWG name, row name, parameters for value
    detectIonsWriteToFileSignal = QtCore.pyqtSignal(str) #args: name of the file
    ...
    def saveCamera(self):
        """saveCamera()
        Saves the current camera data.

        This is the equivalent of clicking the save button in the camera ui."""
        self.saveCameraSignal.emit()

    @scriptFunction()
    def startAcquireCamera(self):
        """startAcquireCamera()
        Starts the acquisition of the camera. Call stopAcquireCamera() to stop the acquisition.

        This is the equivalent of clicking the acquire button in the camera ui to start it."""
        self.startAcquireCameraSignal.emit()

    @scriptFunction()
    def stopAcquireCamera(self):
        """stopAcquireCamera()
        Stops the acquisition of the camera. Call startAcquireCamera() to begin the acquisition before calling this function.

        This is the equivalent of clicking the acquire button in the camera ui to stop it."""
        self.stopAcquireCameraSignal.emit()

    @scriptFunction()
    def detectIonsCamera(self):
        """detectIonsCamera()
        Starts detecting ions from the camera photo.

        This is the equivalent of clicking the detect ions button in the camera ui."""
        self.detectIonsCameraSignal.emit()

    @scriptFunction()
    def detectIonsWriteToFile(self, name):
        """detectIonsWriteToFile(name)
        Starts detecting ions from the camera photo. Then writes the ion positions to the filename specified.

        Args:
            name (str): name of the file"""
        self.detectIonsWriteToFileSignal.emit(name)

    @scriptFunction()
    def checkAcquireWithCamera(self, toggle):
        """checkAcquireWithCamera(toggle)
        Checks the Acquire with Camera box.

        Args:
            toggle (bool): This is the state of the camera box.

        This is the equivalent of clicking the acquire with camera box in the ecperiment ui."""
        self.checkAcquireWithCameraSignal.emit(toggle)

    @scriptFunction()
    def setFileNameCamera(self, name):
        """setFileNameCamera(name)
        Sets the filename for the save file of the camera.

        This is the equivalent of clicking the set file name button in the camera ui.

        Args:
            name (str): name of the file being saved"""
        self.setFileNameCameraSignal.emit(name)
    ...
    @scriptFunction()
    def initializeAWG(self, awgName):
        """ intializeAWG(awgName)
        Initializes the AWG. Equivalent to pressing reintialize AWG.

        Args:
            awgName (str): name of the AWG settings to set

        Raises:
            ScriptException: if there is no AWG settings by that name"""
        self.initializeAWGSignal.emit(awgName)

    @scriptFunction()
    def setAWGVariable(self, awgName, name, value):
        """setAWGVariable(awgName, name, value)
        Sets the values of the row in the AWGTableModel.

        Args:
            awgName (str): name of the AWG settings to set
            name (str): name of the variable
            value(str): parameter for value
        Raises:
            ScriptException: The row does not exist"""
        self.setAWGVariableSignal.emit(awgName, name, value)

    @scriptFunction()
    def checkAWGRow(self, awgName, rowName):
        """ checkAWGRow(awgName, rowName)
        Enables the AWG row. Equivalent to checking AWG enabled column.
        WARNING: THIS ONLY WORKS FOR CHANNEL 0. DOES NOT WORK FOR OTHER CHANNELS.

        Args:
            awgName (str): name of the AWG settings to set
            rowName (str): name of the row you want to toggle
        Raises:
            ScriptException: if there is no AWG settings by that name
            RowException: if there is no row by that name"""
        self.checkAWGRowSignal.emit(awgName, rowName)

    @scriptFunction()
    def uncheckAWGRow(self, awgName, rowName):
        """ uncheckAWGRow(awgName, rowName)
        Enables the AWG row. Equivalent to unchecking AWG enabled column.
        WARNING: THIS ONLY WORKS FOR CHANNEL 0. DOES NOT WORK FOR OTHER CHANNELS.
        ...
        Raises:
            ScriptException: if there is no AWG settings by that name
            RowException: if there is no row by that name"""
        self.uncheckAWGRowSignal.emit(awgName, rowName)
    ...
    @scriptFunction()
    def setAnalysisInitial(self, row, value):
        """setAnalysisInitial(row, value)
        set the variable's inital condition in analysis control to parameter

        This is equivalent to setting the start parameter in analysis control.

        Args:
            row (int): The row the variable is in
            value (float): The value you want to assign the variable"""
        self.setAnalysisInitialSignal.emit(row, value)
    ...
    @scriptFunction()
    def checkTrace(self, traceName):
        """checkTrace(traceName)
        Checks the trace. Equivalent to checking the trace with the checkbox.

        Args:
            traceName (str): name of trace to check.
        """
        self.checkTraceSignal.emit(traceName)

    @scriptFunction()
    def uncheckTrace(self, traceName):
        """uncheckTrace(traceName)
        Unchecks the trace. Equivalent to unchecking the trace with the checkbox.

        Args:
            traceName (str): name of trace to uncheck.
        """
        self.uncheckTraceSignal.emit(traceName)
    ...
    @scriptFunction()
    def checkPulseVariable(self, name, toggle):
        """checkPulseVariable(name, toggle)
        This sets a parameter in the pulse program.

        Args:
            name (str): this is the name of the variable that is changed

            toggle (int): Set this to either QtCore.Qt.Checked(2) or QtCore.Qt.Unchecked(0)"""
        self.checkPulseVariableSignal.emit(name, toggle)

    @scriptFunction()
    def commentOutPulseLines(self, startLine, endLine):
        """commentOutPulseLines(startLine, endLine)
        This allows an individual to comment out lines beginning at start line inclusivly and ending at the endline.

        Args:
            startLine (int): First line to get commented out

            endLine (int): Last line to get commented out. Comments out everything between startline and endline."""
        self.commentOutPulseLinesSignal.emit(startLine, endLine)

    @scriptFunction()
    def unCommentOutPulseLines(self, startLine, endLine):
        """unCommentOutPulseLines(startLine, endLine)
        This allows an individual to uncomment out lines beginning at start line inclusivly and ending at the endline.

        Args:
            startLine (int): First line to get uncommented out

            endLine (int): Last line to get uncommented out. Uncomments out everything between startline and endline."""
        self.unCommentOutPulseLinesSignal.emit(startLine, endLine)
