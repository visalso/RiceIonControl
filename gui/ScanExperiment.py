...
from AWG import AWGDevices,AWGUi, AWGWaveform
...
class ScanExperiment(ScanExperimentForm, MainWindowWidget.MainWindowWidget):
    ...
    def __init__(self, settings, pulserHardware, globalVariablesUi, experimentName, toolBar = None, parent = None,
                 measurementLog = None, camera = None, callWhenDoneAdjusting = None, dbConnection = None, preferences
                 = None, interlock = None, checkBoxUseCamera = None):
        ...
        self.cameraPlotEnabled = self.project.isEnabled('hardware', 'Andor Camera')
        self.unsavedTraceCount = 0
        self.stash = list()
        self.dbConnection = dbConnection
        self.camera = camera
        self.cameraEnabled = True if self.camera else False
        self.checkBoxUseCamera = checkBoxUseCamera
    ...
    def setupUi(self, MainWindow, config):
        ...
        self.requiredPlotNames = ["Scan Data", "Histogram"]
        if self.timestampsEnabled:
            self.requiredPlotNames.append("Timestamps")
        if self.cameraPlotEnabled:
            self.requiredPlotNames.append("Camera")
        if self.experimentName+'.plotNames' in self.config:
            ...
            plotNames = copy.deepcopy(self.requiredPlotNames)     # if we are starting a new project we need to change to plotNames = copy.deepcopy(self.requiredPlotNames)
    ...
    def startScan(self):
        ...
        if self.progressUi.state in [self.OpStates.idle, self.OpStates.starting, self.OpStates.stopping, self.OpStates.running, self.OpStates.paused, self.OpStates.interrupted]:
            ...
            if self.scanControlWidget.getScan().scanTarget == 'Keysight M3202A AWG' and self.scanControlWidget.getScan().scanMode == 0:    # E. Birckelbaw - Populates Keysight Preprocess Cache with waveforms before scan to save time, scanMode 0 is parameter scan
                awgDevice = list(self.scanTargetDict['Keysight M3202A AWG'].values())[0].device
                if awgDevice.settings.deviceSettings['preprocessWaveforms']:
                    logger.info("Preprocessing Waveforms for Keysight M3202A AWG")
                    for step in range(len(self.scanControlWidget.getScan().list)):          #Step scanned parameter through values it will have during scan
                        # if self.scanControlWidget.getScan().scanTarget == 'Global':
                        #     self.globalVariables.globalDict[self.scanControlWidget.getScan().scanParameter].value = self.scanControlWidget.getScan().list[step]
                        awgDevice.settings.varDict[self.scanControlWidget.getScan().scanParameter]['value'] = self.scanControlWidget.getScan().list[step]
                        awgDevice.Preload()                                                 #Evaluate the waveform at each point - adding data to cache
                    awgDevice.settings.varDict[self.scanControlWidget.getScan().scanParameter]['value'] = self.scanControlWidget.getScan().list[0]  #Reset scanned parameter to initial value

            for awgName in list(AWGDevices.AWGDeviceDict.keys()):                       # AWG "Program on scan start"
                if self.project.isEnabled('hardware', awgName):
                    ...
                    if awgDevice.settings.deviceSettings['programOnScanStart']:
                        ...
                        if self.scanControlWidget.getScan().scanTarget == 'Keysight M3202A AWG' and awgDevice.settings.deviceSettings['preprocessWaveforms'] and self.scanControlWidget.getScan().scanMode == 0:  # scanMode 0 is parameter scan
                            awgDevice.immediate_program(0) # 0: To immediate_program scan step 0
                        else:
                            awgDevice.program()
            ...
            self.checkBoxUseCamera.setEnabled(False)
            if self.cameraEnabled and self.checkBoxUseCamera.isChecked():
                if self.camera.actionAcquire.isChecked() == False:
                    self.camera.toggleAcquire()
                # Wait for camera to get set up, break if timeout
                TIMEOUT = 10
                start_time = time.time()
                while not self.camera.ready:
                    if time.time() - start_time > TIMEOUT:
                        print("ERROR: Timeout while waiting for camera to start")
                        return self.onStop()
    ...
    def onStop(self, reason='stopped'):
        if self.progressUi.state in [self.OpStates.starting, self.OpStates.running, self.OpStates.paused, self.OpStates.interrupted, self.OpStates.stopping ]:
            ...
            if self.cameraEnabled and self.checkBoxUseCamera.isChecked():
                if self.camera.actionAcquire.isChecked() == True:
                    self.camera.toggleAcquire()
            self.checkBoxUseCamera.setEnabled(True)
    ...
    def processData(self, data, queue_size):
        ...
        for awgName in list(AWGDevices.AWGDeviceDict.keys()):               # E. Birck - AWG "Program on Scan Steps"
                if self.project.isEnabled('hardware', awgName):
                    awgDevice = list(self.scanTargetDict[awgName].values())[0].device
                    if awgDevice.settings.deviceSettings['reprogramOnScanSteps'] and self.scanControlWidget.getScan().scanMode == 0 and self.scanControlWidget.getScan().scanTarget == awgName: # scanMode 0 is parameter scan
                        logging.getLogger(__name__).info("Programming {0}".format(awgName))
                        if awgDevice.settings.deviceSettings['preprocessWaveforms'] and self.scanControlWidget.getScan().scanTarget == 'Keysight M3202A AWG':
                            if self.context.currentIndex != (len(self.scanControlWidget.getScan().list)-1):     # If not last index
                                awgDevice.immediate_program(self.context.currentIndex + 1)                      # Program next index
                        else:
                            if self.context.currentIndex != (len(self.scanControlWidget.getScan().list)-1):     # If not last index
                                awgDevice.settings.varDict[self.scanControlWidget.getScan().scanParameter]['value'] = self.scanControlWidget.getScan().list[self.context.currentIndex + 1]
                                awgDevice.program()
    ...
    def finalizeData(self, reason='end of scan'):
        if not self.context.dataFinalized:  # is not yet finalized
            ...
            for awgName in list(AWGDevices.AWGDeviceDict.keys()):
                if self.project.isEnabled('hardware', awgName):
                    awgDevice = list(self.scanTargetDict[awgName].values())[0].device
                    if awgDevice.settings.deviceSettings['preprocessWaveforms'] and awgDevice.displayName == 'Keysight M3202A AWG':
                        logging.getLogger(__name__).info("Unloading Pre-loaded Data from {0}".format(awgName))
                        awgDevice.Unload_data()
