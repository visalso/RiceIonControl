...
class ExperimentUi(WidgetContainerBase,WidgetContainerForm):
    ...
    def setupUi(self, parent):
        ...
        if self.AndorCameraEnabled:
            from Camera.camera import Camera, camandor
            from Camera import camera, fileSettings
            # TODO: Switch back to enable camera detection decisions.
            #if camandor.error:
            #    self.AndorCameraEnabled = False
            self.useCamDataEnabled = self.AndorCameraEnabled
        ...
        for widget, name in [(ScanExperiment.ScanExperiment(self.settings, self.pulser, self.globalVariablesUi,
                                                            ...
                                                            checkBoxUseCamera=self.checkBoxUseCamera),
                              "Scan")
                             ]:
...
#        self.tabifyDockWidget( self.DDS9910DockWidget, self.globalVariablesDock )
        ...
        if self.AndorCameraEnabled:
            self.actionCamera.triggered.connect(self.onCamera)
        else:
        ...
        # Setup Camera UI
        if self.AndorCameraEnabled:
            self.CameraWindow = Camera(self.config, self.globalVariablesUi, self.shutterUi,
                                       self.scanExperiment, self.pulseProgramDialog, parent=self)
            self.CameraWindow.setupUi(self.CameraWindow)
            self.scanExperiment.camera = self.CameraWindow
            self.scanExperiment.cameraEnabled = self.AndorCameraEnabled
            # self.dedicatedCountersWindow.setCamera(self.CameraWindow)
            import Camera.fileSettings as fileSettings
            fileSettings.basedir = self.project.baseDir
            fileSettings.imagesavepath = self.project.projectDir
        else:
            # disable camera checkbox when there is no camera
            self.checkBoxUseCamera.setChecked(False)
            self.checkBoxUseCamera.setCheckable(False)
