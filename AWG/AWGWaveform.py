...
class AWGWaveform(object):
            ...
            if node.enabled:
                if node.nodeType==nodeTypes.segment:
                    ...
                    for n in range(repetitions):
                        ...
                        sampleList = numpy.append(sampleList, newSamples)
        if "KeysightM3202A" in str(self.settings.deviceProperties['calibration']):  # E. Birckelbaw - Keysight Amplitude Scaling
            if any(sampleList):
                max_amp = numpy.amax(sampleList)
             ###   max_amp = 0.01
                if max_amp > self.settings.deviceSettings['voltlimit'].magnitude:
                    logger = logging.getLogger(__name__)
                    logger.warning("AWG Signal Exceeds Maximum Amplitude! Current Amplitude: %s Maximum Amplitude: %s" % (max_amp, self.settings.deviceSettings['voltlimit'].magnitude))
                    logger.warning("AWG Signal Scaled Down by %s to Maintain Max Amplitude!" % (max_amp / self.settings.deviceSettings['voltlimit'].magnitude))
                    sampleList = sampleList * (self.settings.deviceSettings['voltlimit'].magnitude / max_amp)
    ...
    def evaluateEquation(self, node, duration, startStep):
        ...
        and the length is constrained to self.settings.cacheDepth. A key to the OrderedDict is an equation string, e.g. "7*sin(5*t)+3".
        ...
        if not error:
            ...
            if 'lt0' in node.equation:
                varValueDict['lt0'] = (startStep / self.sampleRate.m_as('Hz'))
    ...
    def computeFunction(self, sympyExpr, tVar, startStep, stopStep):
        ...
        if numSamples <= 0:
            ...
            if "KeysightM3202A" in str(self.settings.deviceProperties['calibration']):  # E. Birckelbaw - Speed Optimization
                sampleList = []
                t = numpy.linspace(start=(startStep/self.sampleRate.m_as('Hz')), stop=(stopStep/self.sampleRate.m_as('Hz')), num=numSamples, endpoint=True)
                lt = numpy.linspace(start=(0/self.sampleRate.m_as('Hz')), stop=((numSamples-1)/self.sampleRate.m_as('Hz')), num=numSamples, endpoint=True)
                test = "sampleList.append(%s)" % str(sympyExpr).replace('tanh', 'numpy.tanh').replace('exp', 'numpy.exp').replace('sin', 'numpy.sin').replace('cos','numpy.cos').replace('pi','numpy.pi') # Can be appended with ".replace('function', 'numpy.command')
                exec(test)
                return sampleList[0]
                #sampleList = [0]*10 # Harvey Kaplan_ In case the control program does not respond because it is plotting too much. Comment the 5 lines above, uncomment these two lines, start the program, unckeck 'plot', and then restart the program with the comments back how they started.
                #return sampleList
            else:
