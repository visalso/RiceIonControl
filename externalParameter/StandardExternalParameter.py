...
if visaEnabled:
    ...
    class AD5781Arduino(ExternalParameterBase):
        """
        Set the stable voltages on the two-channel Arduino controller for AD5781 18-bit voltage references. (ChipTrap)
        """
        className = "2-channel AD5781 (Arduino)"
        _outputChannels = {"Voltage Ch1": "", "Voltage Ch2":""}
        def __init__(self, name, config, globalDict, instrument='ASRL13::INSTR'): #Check that this is the right instrument
            logger = logging.getLogger(__name__)
            ExternalParameterBase.__init__(self, name, config, globalDict)
            logger.info( "trying to open '{0}'".format(instrument) )
            self.rm = visa.ResourceManager()
        ...
        def setValue(self, channel, v):
            # function, unit, suffix= self._outputLookup[channel]
            if v>0.990 or v<0:
                print("Voltage too high: V1= set to 0.987 V")
                v=0.989
            if channel == "Voltage Ch1":
                vOut1 = (v/10)*(2**18-1)
                command = 'V1 '+ str(int(vOut1))+'\n'
            else:
                vOut2 = (v/10)*(2**18-1)
                command = 'V2 '+ str(int(vOut2))+'\n'
#            command = "{0}{1}{2}".format(function, int(v.m_as(unit)), suffix)
            self.instrument.write(command)
            return v
