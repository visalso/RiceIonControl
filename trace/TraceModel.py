...
class TraceModel(CategoryTreeModel):
    ...
    def checkboxChangeScript(self, index, value):
        """Plot or unplot the plottedTrace at the given index based on the checkbox for the script"""
        node = self.nodeFromIndex(index)
        plottedTrace = node.content
        success = False
        if (plottedTrace.isPlotted is False and value == QtCore.Qt.Unchecked):
            return success
        if (value == QtCore.Qt.Unchecked) and (plottedTrace.curvePen > 0) and plottedTrace.isPlotted:
            plottedTrace.plot(0) #unplot if unchecked
            success=True
        elif len(plottedTrace.traceCollection.x) != 0: #Make sure the x array isn't empty before trying to plot
            plottedTrace.plot(-1)
