...
class VariableTableModel(QtCore.QAbstractTableModel):
    ...
    def setVarEnabledScript(self, name, value):
        """This function enables the variable in the script"""
        if self.variabledict.__contains__(name) is True:
            index = self.variabledict.getPosition(name)
            self.variabledict.setEnabledIndex(index, value == QtCore.Qt.Checked)
            self.dataChanged.emit(self.createIndex(index, 0), self.createIndex(index, 4))
            self.recalculateDependent(self.variabledict.keyAt(index))
            self.contentsChanged.emit()
            return True
        else:
            return False
